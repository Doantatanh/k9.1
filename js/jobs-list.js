...
async function loadJobsList(page = 1) {
  try {
    ...
    const apiUrl = `${config.getUrl("JOBS_LIST")}?pageNumber=${page}&pageSize=${limit}`;
    ...
    // API này không trả về success, chỉ cần kiểm tra có data hay không
    if (!result.data) {
    ...
    const totalPages = result.totalPages || Math.ceil((result.totalRecords || 0) / limit);

    if (jobsList.length === 0) {
      container.innerHTML = `<div class="col-12 text-center py-5"><h3 style="color: #666;">Hiện tại chưa có vị trí tuyển dụng nào.</h3></div>`;
    ...
    console.error("Lỗi load danh sách Tuyển dụng từ API:", error);
  }
}
...
function renderJobs(jobsList) {
  ...
  jobsList.forEach((item) => {
    ...
    const salary = item.salary || "Thỏa thuận";
    const image = config.getImgUrl(
      item.avatarUrl || "/images/resource/news-1.jpg",
    ); // Fallback image
    ...
    // Link chi tiết (Có thể tạo file job-detail.html sau hoặc dùng chung article.html nếu cấu trúc phù hợp)
    const detailLink = `job-detail.html?id=${id}`;

    html += `
                <div class="portfolio-block-four col-xl-3 col-lg-4 col-md-6 col-sm-12">
                        <div class="inner-box">
                            <div class="image-box">
                                <figure class="image">
                                    <img src="${image}" alt="${title}" style="height: 250px; object-fit: cover;">
                                </figure>
                                <div class="hover-effect">
                                    ...
                                    <a href="${detailLink}" class="link"></a>
                                </div>
                            </div>
                            <div class="lower-content" style="padding: 15px;">
                                <h4 class="name" style="style="margin-bottom: 15px; min-height: 10px; text-align: justify; font-size: 18px;"">
                                    <a href="${detailLink}" style="color: #2a5caa; font-weight: bold;">${title}</a>
                                </h4>
                                <div class="info" style="font-size: 16px; color: #5d677a; line-height: 1.8; font-weight: 500;">
                                    <div style="color: #d73f34;"><span class="fa fa-hand-holding-usd"></span> Lương: ${salary}</div>  
                                    <div><span class="fa fa-map-marker"></span> ${location}</div>
                                    <div><span class="fa fa-clock"></span> Hình thức: ${jobType}</div>
                                    <div><span class="fa fa-calendar"></span> Hạn nộp: ${deadline}</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
function renderPaginationNumbers(totalPages) {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    btn.onclick = () => goToPage(i); // Sử dụng hàm goToPage
