// 1. Đọc số trang từ URL khi trang web vừa load
const params = new URLSearchParams(window.location.search);
let currentPage = parseInt(params.get("page")) || 1;
const limit = 10; // Bạn có thể đổi lại thành 10 nếu muốn
...
async function loadNewsList(page = 1) {
  try {
    ...
    const apiUrl = `${config.getUrl("NEWS_LIST")}?ArticleType=Tin%20t%E1%BB%A9c&PageIndex=${page}&PageSize=${limit}`;
    ...
    if (newsList.length === 0) {
      container.innerHTML = `<div class="col-12 text-center py-5"><h3 style="color: #666;">Hiện tại chưa có tin tức nào.</h3></div>`;
