...
async function loadWorkshopList() {
  try {
    ...
    // ArticleType=Workshop
    const apiUrl = `${config.getUrl("NEWS_LIST")}?ArticleType=Workshop&PageIndex=${currentPage}&PageSize=${limit}`;
    ...
    const totalPages = result.pagination ? result.pagination.totalPages : Math.ceil((result.count || 0) / limit);

    if (workshopList.length === 0) {
      container.innerHTML = `<div class="col-12 text-center py-5"><h3 style="color: #666;">Hiện tại chưa có Workshop nào.</h3></div>`;
    ...
    console.error("Lỗi load danh sách Workshop từ API:", error);
  }
}

// Hàm này giúp cập nhật URL và tải dữ liệu mới
function goToPage(page) {
  currentPage = page;

  // Cập nhật URL (vd: ?page=2)
  const newUrl = `${window.location.pathname}?page=${page}`;
  window.history.pushState({ path: newUrl }, '', newUrl);

  loadWorkshopList(page);
  window.scrollTo({ top: 300, behavior: "smooth" });
}

function renderWorkshopList(workshopList) {
  const config = window.API_CONFIG;
  const container = document.getElementById("workshop-list");
  let html = "";
  workshopList.forEach((item) => {
    const id = item.cmsArticleId;
    const title = item.title || "";
    const image = config.getImgUrl(item.presentationImage);
    const date = item.createdAt
      ? new Date(item.createdAt).toLocaleDateString("vi-VN")
      : "";
    // Link chi tiết bài viết (Dùng chung article.html cho cả News và Workshop)
    const detailLink = `article.html?id=${id}`;
                ...
                <div class="portfolio-block-four col-xl-3 col-lg-4 col-md-6 col-sm-12">
                        <div class="inner-box">
                            ...
                            <div class="lower-content">
                                <h4 class="name" style="margin-bottom: 15px; min-height: 10px; text-align: left; font-size: 18px;">
                                    <a href="${detailLink}" style="color: #333; font-weight: bold;">${title}</a>
                                </h4
                            </div>
                        </div>
                    </div>
