async function loadComboQuaTang() {
  try {
    ...
    const apiUrl = config
      ? config.getUrl("PRODUCT_LIST") + "?categoryId=4&domainId=1&pageIndex=1&pageSize=20"
      : "http://macaron.a.csoftlife.com/api/v1/CoreProduct/list?categoryId=4&domainId=1&pageIndex=1&pageSize=20";
    ...
    console.error("Lỗi load Combo Quà Tặng từ API:", err);
  }
}

// Gọi hàm khi trang đã tải xong
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", loadComboQuaTang);
} else {
  loadComboQuaTang();
}
